...
To upgrade, close the game, extract the newest release ZIP into a fresh folder and run its Install.cmd. Do not run an installer from an older download. Uninstalling first is unnecessary. Settings → Accessibility shows the installed Mod version and offers a manual GitHub update check (click Mod info, updates). Beta releases are included; updates are never installed automatically.
